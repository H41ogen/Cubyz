...
pub const KeyBoard = struct { // MARK: KeyBoard
	const c = Window.c;
	pub var structuredKeys = .{
		.@"Display" = [_]Window.Key{
			.{.name = "fullscreen", .key = c.GLFW_KEY_F11, .pressAction = &Window.toggleFullscreen},
			.{.name = "hideGui", .key = c.GLFW_KEY_F1, .pressAction = &toggleHideGui},
	...
	pub var keys = collapseKeys();

	inline fn collapseKeys() []const *Window.Key {
		comptime var size: usize = 0;
		inline for(@typeInfo(@TypeOf(structuredKeys)).@"struct".fields) |field| {
			const array = @field(structuredKeys, field.name);
			size += array.len;
		}
		comptime var result: [size]*const Window.Key = undefined;
