...
const SliderWithTextInput = @This();
...
pub fn init(pos: Vec2f, width: f32, labelText: []const u8, minValue: f32, maxValue: f32, initialValue: f32, callback: *const fn(f32) void) *SliderWithTextInput {
	const label = Label.init(undefined, width - 3*border, labelText, .center);
	const button = Button.initText(.{0, 0}, undefined, "", .{});
	const self = main.globalAllocator.create(SliderWithTextInput);
	self.* = SliderWithTextInput{
		.pos = pos,
		.size = undefined,
		.minValue = minValue,
		.maxValue = maxValue,
...
pub fn deinit(self: *const SliderWithTextInput) void {
	self.label.deinit();
	self.button.deinit();
	main.globalAllocator.destroy(self);
}

pub fn toComponent(self: *SliderWithTextInput) GuiComponent {
	return .{.sliderWithTextInput = self};
}

fn setButtonPosFromValue(self: *SliderWithTextInput) void {
...
fn updateValueFromButtonPos(self: *SliderWithTextInput) void {
	const range: f32 = self.size[0] - 3*border - self.button.size[0];
	const len: f32 = self.maxValue - self.minValue;
...
pub fn updateHovered(self: *SliderWithTextInput, mousePosition: Vec2f) void {
...
pub fn mainButtonPressed(self: *SliderWithTextInput, mousePosition: Vec2f) void {
	if(GuiComponent.contains(self.button.pos, self.button.size, mousePosition - self.pos)) {
...
pub fn mainButtonReleased(self: *SliderWithTextInput, _: Vec2f) void {
	self.button.mainButtonReleased(undefined);
}

pub fn render(self: *SliderWithTextInput, mousePosition: Vec2f) void {
	texture.bindTo(0);
	Button.pipeline.bind(draw.getScissor());
	draw.setColor(0x40000000);
